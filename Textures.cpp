...
glm::vec3 ImageTexture::texture(glm::vec2 uv){
    int x = (int) (round(TEXTURE_REPETITION * uv.x * this->base_color.width)) % ( this->base_color.width );
    int y = (int) (round(TEXTURE_REPETITION * uv.y * this->base_color.height)) % (this->base_color.height);
    int index = (y * this->base_color.width + x) * 4;

    glm::vec3 ret = glm::vec3(
            (float) this->base_color.data[index + 0]/255,
